...
use crate::commands::{is_claude_configured, is_cursor_configured, is_gemini_configured};
...
pub fn run_doctor() {
    ...
    match SecureKeyring::set(test_key, test_value) {
        Ok(_) => match SecureKeyring::get(test_key) {
    ...
    match claude_settings {
        Some(path) if path.exists() => {
            if is_claude_configured() {
                ...
                println!(
                    "{} exists but no veto hooks (run `veto setup claude`)",
                    "○".yellow()
                );
            }
        }
    ...
    let gemini_settings = dirs::home_dir().map(|h| h.join(".gemini").join("settings.json"));
    ...
    match gemini_settings {
        Some(path) if path.exists() => {
            if is_gemini_configured() {
                ...
                println!(
                    "{} exists but no veto hooks (run `veto setup gemini`)",
                    "○".yellow()
                );
            }
        }
    ...
    let cursor_hooks = dirs::home_dir().map(|h| h.join(".cursor").join("hooks.json"));
    ...
    match cursor_hooks {
        Some(path) if path.exists() => {
            if is_cursor_configured() {
                ...
                println!(
                    "{} exists but no veto hooks (run `veto setup cursor`)",
                    "○".yellow()
                );
            }
        }
